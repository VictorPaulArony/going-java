import java.net.URI;
import java.util.HashMap;
import java.util.Map;

public class BreakdownURL {
    public Map<String, String> parseURL(String url) {
        Map<String, String> res = new HashMap<>();

        try {
            URI inUrl = new URI(url);

            res.put("protocol", inUrl.getScheme());
            res.put("domain", inUrl.getHost());
            int port = inUrl.getPort();
            if (port != -1) {
                res.put("port", String.valueOf(port));
            }
            res.put("path", inUrl.getPath() == null ? "/" : inUrl.getPath());
            String query = inUrl.getQuery();
            if (query != null && !query.isEmpty()) {
            res.put("query", inUrl.getQuery());
            }

        } catch (Exception e) {
             res.put("error", "Invalid URI: " + e.getMessage());
        }
        return res;
        
    }

    public static void main(String[] args) {
        BreakdownURL parser = new BreakdownURL();

        // Test case 1
        String URL1 = "https://www.example.com:8080/path?name=value";
        Map<String, String> components1 = parser.parseURL(URL1);
        System.out.println("Components of URL 1: " + components1);

        // Test case 2
        String URL2 = "http://example.com/";
        Map<String, String> components2 = parser.parseURL(URL2);
        System.out.println("Components of URL 2: " + components2);
    }
}